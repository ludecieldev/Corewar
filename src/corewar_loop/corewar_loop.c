...
static void run_instruction(corewar_t *corewar, champion_t *champ)
{
    if (champ->alive == 0)
        return;
    for (int i = 0; i < champ->process_nb; i++) {
...
static void load_param(process_t *process)
{
    process->param->param_info = *((u_char *)get_address(process->mem,
        process->pc));
    process->pc++;
    for (int i = 0; i < op_tab[process->param->instr - 1].nbr_args; i++) {
...
static void update_champ(corewar_t *corewar, champion_t *champ)
{
    if (champ->alive == 0)
        return;
    for (int i = 0; i < champ->process_nb; i++) {
